...
class Cartridge(Resource):
    ...
    parser.add_argument('cartridgeId',
                        type=str,
                        required=True,
                        help="This field cannot be left blank!"
                        )
    parser.add_argument('testStatus',
                        type=int,
                        required=True
                        )
    parser.add_argument('departmentName',
                        type=int,
                        required=True
                        )
    ...
    parser.add_argument('hospitalName',
                        type=int,
                        required=True
                        )
    parser.add_argument('operatorName',
                        type=int,
                        required=True
                        )
    ...
    def post(self, cartridgeId):
        if CartridgeModel.find_by_name(cartridgeId):
            return {'message': "An cartridge with name '{}' already exists.".format(cartridgeId)}, 400
